...
class PNETCDF : public ArrayIODriver {
public: 
    PNETCDF(const std::string fileName, bool readOnly=true, const std::string indexPath="", MPI_Comm comm=MPI_COMM_WORLD);
    ~PNETCDF();

    /*!
      \brief Check if the file is opened successfully.
    */
    virtual bool isValid() const {return (_ncid<0?false:true);}
...
private:
    ...
    // return the PNETCDF object type at path location
    //H5O_type_t __getPNETCDFObjType(const std::string &path);

    // return -1 if the type is not supported by PNETCDF
    nc_type __getPNETCDFDataType(const FQ::DataType fqType);
    ...
    // pnetCDF does not support deletion
